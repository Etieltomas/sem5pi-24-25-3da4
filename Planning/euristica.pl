...
surgery(so2,45,51,45).
surgery(so3,45,86,45).
surgery(so4,45,75,45).

%surgery_id(OpCode,OpType,Priority).
surgery_id(so100001,so2, 'Low').
surgery_id(so100002,so3, 'Low').
surgery_id(so100003,so4, 'Low').
surgery_id(so100004,so2, 'Low').
surgery_id(so100005,so4, 'Low').
surgery_id(so100006,so4, 'Low').
...
schedule_all_surgeries(Room,Day):-
    ...
    findall(OpCode,surgery_id(OpCode,_,_),LOpCode),
...
% Sort surgeries by priority (High > Medium > Low)
sort_surgeries_by_priority(Surgeries, SortedSurgeries) :-
    write('Before: '),write(Surgeries), nl,
    predsort(compare_priority, Surgeries, SortedSurgeries),
    write('After:'),write(Surgeries), nl,


compare_priority(<, (_, _, 'High'), (_, _, _)).
compare_priority(>, (_, _, _), (_, _, 'High')).
compare_priority(<, (_, _, 'Medium'), (_, _, 'Low')).
compare_priority(>, (_, _, 'Low'), (_, _, 'Medium')).
compare_priority(<, (_, _, 'High'), (_, _, 'Medium')).
compare_priority(>, (_, _, 'Medium'), (_, _, 'High')).
compare_priority(<, (_, _, Priority), (_, _, Priority)).
...
availability_operation(OpCode,Room,Day,LPossibilities,LDoctors):-surgery_id(OpCode,OpType,_),surgery(OpType,_,TSurgery,_),
...
obtain_better_sol1(Room,Day):-
    ...
    findall((OpCode, Duration, Priority), surgery_id(OpCode, Duration, Priority), Surgeries),!,
    %permutation(Surgeries,SortedSurgeries),
    sort_surgeries_by_priority(Surgeries, SortedSurgeries),
    findall(OpCode, member((OpCode, _, _), SortedSurgeries), LOpCode),
    retractall(agenda_staff1(_,_,_)),
    retractall(agenda_operation_room1(_,_,_)),
    retractall(availability(_,_,_)),
    findall(_,(agenda_staff(D,Day,Agenda),assertz(agenda_staff1(D,Day,Agenda))),_),
    ...
    % As cirurgias já estão ordenadas em LOpCode
