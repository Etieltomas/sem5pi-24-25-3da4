...
generate(FinalGeneration):-
    ...
    findall((D, Day, Agenda), agenda_staff1(D, Day, Agenda), StaffList),
    ...
    retractall(agenda_staff1(_,Day,_)),
    retractall(availability(_,Day,_)),
    reassert_agenda_staff1(StaffList),
    ...
    generate_generation(0,NG,PopOrd,FinalGeneration).

reassert_agenda_staff1([]).
reassert_agenda_staff1([(D, Day, Agenda) | Rest]) :-
    assertz(agenda_staff1(D, Day, Agenda)),
    reassert_agenda_staff1(Rest).

generate_population(Pop):-
    population(PopSize),
    surgeries(NumT),
    room(Room),
    findall(Surgery,room_surgery(Room, Surgery),SurgeriesList),
...
generate_generation(G,G,Pop, FinalGeneration):-!,
...
generate_generation(N, G, Pop, FinalGeneration) :-
    ...
    N1 is N + 1,
    generate_generation(N1, G, NPopOrd2, FinalGeneration)
))).
